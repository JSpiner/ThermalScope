...
void invalidateVideo() {
  ...
  if (scaleMode == SCALE_MODE_BLI) {
    for (int i = 0; i < leptonWidth * videoScale; i+=(videoScale * 2 - 1)) {
      for (int j = 0; j < leptonHeight * videoScale; j+=(videoScale * 2 - 1)) {

        uint16_t originValues[4] = {
          renderData[i][j],
          renderData[i][j+(videoScale * 2 - 1)],
          renderData[i+(videoScale * 2 - 1)][j+(videoScale * 2 - 1)],
          renderData[i+(videoScale * 2 - 1)][j+(videoScale * 2 - 1)],
        };
  ...
  for (int i = 0; i < leptonWidth * videoScale; i++) {
    for (int j = 0; j < leptonHeight * videoScale; j++) {      
      uint16_t color = tft.color565(
        colormap_rainbow[renderData[i][j] * 3 + 0],
        colormap_rainbow[renderData[i][j] * 3 + 1],
        colormap_rainbow[renderData[i][j] * 3 + 2]
      );
