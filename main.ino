...
uint16_t renderData[leptonWidth * leptonHeight];
  ...
  for (int i = 0; i < leptonWidth; i++){
    for (int j = 0; j < leptonHeight; j++){
      ...
      uint16_t color = tft.color565(
        colormap_rainbow[pixelData * 3 + 0],
        colormap_rainbow[pixelData * 3 + 1],
        colormap_rainbow[pixelData * 3 + 2]
      );
      renderData[i * leptonHeight + j] = color;
    }
  }
